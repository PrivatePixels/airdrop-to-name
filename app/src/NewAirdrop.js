...
function NewAirdrop() {
  ...
  useEffect(()=>{
    ...
    reader.onload = async (e)=>{
      ...
      setData(merklize(recipients, nameField, amountField))
    }
  ...
  return (
    <React.Fragment>
      ...
      <Info style={{"margin-bottom": "10px"}}>csv header fields should be <strong>{nameField}</strong> and <strong>{amountField}</strong>. <Button onClick={()=>{}} size="mini">Change this</Button></Info>
      ...
      {data && ( hash
        ? <Info style={{"margin-bottom": "10px"}}>You're data with merkle root ({data.root}) and ipfs hash ({hash}) has been added to ipfs but may need to propagate through the network if it doesn't already appear <a href={`https://ipfs.eth.aragon.network/ipfs/${hash}`} target="_blank">here</a>.</Info>
        : <Info.Alert style={{"margin-bottom": "10px"}}>no ipfs hash generated. missing ipfs node?</Info.Alert> )}
      <Field>
